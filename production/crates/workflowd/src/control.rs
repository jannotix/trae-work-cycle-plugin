...
pub fn execute(
    ...
    if operation == ControlOperation::Limits {
        return limits();
    }
...
fn limits() -> Result<Value, String> {
    ...
    Ok(json!({
        "admission": {
            "diskReserveBytes": 2_u64 * 1024 * 1024 * 1024,
            "leaseMillis": 15_000_u64,
            "maximumActive": logical_cpus.div_ceil(2).clamp(1, 8),
            "maxCpuPercent": 85.0_f64,
            "memoryReserveBytes": 1024_u64 * 1024 * 1024,
            "recoveryAdmissionsPerTick": 1_u64,
            "retryMillis": 500_u64,
        },
        "repair": {
            "maximumRepairCycles": 5_u8,
        },
    }))
}
