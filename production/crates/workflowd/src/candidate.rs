...
const DELIVERY_JOURNAL_DIRECTORY: &str = "trae-cycle-delivery";
const DELIVERY_TEMPORARY_PREFIX: &str = ".trae-cycle-";
...
pub fn delivery_recovery_required(
    ...
    let root = git_directory.join(DELIVERY_JOURNAL_DIRECTORY);
    Ok(root.join(format!("{candidate_id}.json")).try_exists()?
        || root
