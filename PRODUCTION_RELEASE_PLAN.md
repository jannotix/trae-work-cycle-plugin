...
| T05 | In progress — Trae Work Desktop 0.1.61 is pinned after a real in-place update from 0.1.54. The update preserved the task, Command, MCP registration and data directory; a post-update explicit `cycle_setup` call reported all loopback roles configured. The active Skill is updated to 1.0.1 with a verified rollback copy. UI quick/full/restart/uninstall rows remain incomplete while the host UI automation bridge is unstable |
