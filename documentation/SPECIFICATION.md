...
Scope-driven gates are matched against what the plan's write scopes reach as well as what they contain: the code graph is asked which files consume the authorized scopes, to a depth of two, and the reached set is judged by the same layer rules. This can only add gates, never remove one. A reached set wider than the ceiling reports `impact:high-fan-in` naming the most-consumed symbols; a project the index does not hold reports `impact:unresolved` with the command that resolves it. Both are recorded for the reviewers and neither is mandatory, because code intelligence is bound to delivery and a first workflow verifies before its project is indexed.
