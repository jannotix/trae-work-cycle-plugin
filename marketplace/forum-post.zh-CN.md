...
Cycle for Trae Work 是一个本地运行的交付治理插件：它不替代 TRAE 的规划、浏览或评审能力，而是在"改完了"和"真的交付了"之间加一道证据门禁。采用 FSL-1.1-MIT 公平源码许可证（两年后转 MIT）。控制平面、状态与命令执行器都在本机运行，Cycle 没有云端账号或遥测；用户配置的角色端点可以位于云端，并会接收完成该角色所需的受限请求/候选上下文。
...
- **命令同意门禁**：shell、破坏性或发布类命令直接拒绝；其他非预批准命令会先显示精确参数，只有用户逐项确认后才获得 15 分钟、一次性的审计凭证。该同意不是操作系统沙箱，只应批准可信仓库的命令。
...
- Windows x64 控制平面认证：quick / full 自主循环、命令同意、返修路径、37 个工具全量、多项目并发全部通过；
- 50 万文件仓库索引特征：0 解析错误，图 217 万节点 / 170 万边，增量刷新 35 秒，峰值内存 308 MB；已记录的冷索引尚未达到 30 分钟发布门禁，下一生产版本发布前会继续优化；
- 当前 Windows 证据：fmt + clippy -D warnings + 284 个测试全绿；WSL 仍是发布候选必须通过的独立门禁；
