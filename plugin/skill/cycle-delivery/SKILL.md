...
**Architecture** (full mode, and quick mode when the control plane requires a plan). Consult the architect with `cycle_role` operation `architect_consult`, passing the original request, the risk facts and the relevant project context. Compose the returned analysis into an `ArchitecturePlan`: requirements with acceptance criteria, risks, assumptions, integration checks, and an acyclic task DAG where every task has a bounded objective, one write scope set, requirement links, dependencies and its own verification command. Submit with `cycle_submit_architecture`. Fix every validation error the control plane reports and resubmit; never work around validation.

`cycle_submit_architecture` accepts one complete ArchitecturePlan, not prose or partial fragments. Pass it as `plan` when the host supports an object argument; when a host transport serializes tool objects unreliably, pass the same complete JSON value as the `plan_json` string instead. Never send both. Its exact required keys and the required nested `Requirement` and `PlannedTask` fields are in [the tool reference](references/tools.md#architecture-plan-shape). Keep the `request_digest` from `cycle_start` unchanged. A rejected schema means correct the plan and resubmit; never skip the plan or substitute a stub review.

**Execution.** Create the isolated worktree with `cycle_worktree`, passing the user's project directory; every edit happens inside the returned `path`. Run `cycle_index` on the project directory now rather than at delivery: verification reads the code graph to decide which gates a change earns through what it reaches, and an unindexed project can only record that it could not tell. Read tasks with `cycle_tasks`. Pick one ready task, do the work inside its write scopes, run the task's verification command, register the outcome with `cycle_evidence`, and move to the next task. If a task cannot proceed, report `blocked`; if the plan itself is defective, report `plan_defect` via `cycle_execution_report` — do not improvise a different plan.
...
**Arbitration.** Consult the arbiter with `cycle_role` operation `arbiter_verdict`, passing `workflow_id` as well as the request. The control plane reads the recorded reviews from its own store and hands them to the arbiter itself; it refuses the consultation without a `workflow_id`, and refuses a full-mode candidate that does not yet have both reviews. Your request still carries the original request text, the candidate digest and the verification evidence, and reaches the arbiter verbatim. Submit the returned verdict with `cycle_arbitrate`. An approval here, bound to the frozen candidate, is the only approval that authorizes delivery — and an approval that contradicts a reviewer's rejection is recorded, refused by the plane and routed to repair.
