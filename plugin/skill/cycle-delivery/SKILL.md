...
`cycle_submit_architecture` accepts one JSON object, not prose or partial fragments. Its exact required keys and the required nested `Requirement` and `PlannedTask` fields are in [the tool reference](references/tools.md#architecture-plan-shape). Keep the `request_digest` from `cycle_start` unchanged. A rejected schema means correct the object and resubmit; never skip the plan or substitute a stub review.
